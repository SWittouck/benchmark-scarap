...
# infer core genome
/usr/bin/time -v -o $dout/corefull/stats.txt scarap core-pipeline \
  $din_faas $dout/corefull -t $threads -c
/usr/bin/time -v -o $dout/core100/stats.txt scarap core-pipeline \
  $din_faas $dout/core100 -t $threads -c --max_genes 100